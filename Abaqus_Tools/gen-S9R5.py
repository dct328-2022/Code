...
in_file = tuple(open("/home/chenting/ProgrammingCode(NewOnly)/AbaqusS9R5/Job-1.inp","r"))

#out_file = open('Job-1-out_file.inp','w')
out_file = open('/home/chenting/ProgrammingCode(NewOnly)/AbaqusS9R5/Job-1-finale.inp','w')
...
for i in range(len(in_file)):
    if 'name=ShellPanel*Node' in in_file[i]:
        rigaInizioNodi = i + 1
    if '*Elementtype=S8R5' in in_file[i]:
    	rigaFineNodi = i - 1
    	rigaInizioElem = i + 1
...
print(rigaInizioNodi, rigaFineNodi, rigaInizioElem, rigaFineElem)
...
# save nodal coordinates for later computation
	if '*Part, name=ShellPanel *Node' in in_file[i]:
...
# computation of the central node for the S9R5 element
	if '*Element, type=S8R5' in in_file[i]:
