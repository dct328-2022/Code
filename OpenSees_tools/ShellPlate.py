...
Node1 = Node(1, 0, 0, 0)
Node2 = Node(2, 90, 0, 0)
Node3 = Node(3, 90, 100, 0)
Node4 = Node(4, 0, 100, 0)

Node(5, 0, 50, 0.1)

CornerNodes = [Node1, Node2, Node3, Node4]
...
ShellElement.ShellMITC9Meshed(1, 2, 3, 4, 30, 25, 100001, 100001, 10008)
#ShellElement.ShellNLDKGTMeshed(1, 2, 3, 4, 45, 50, 100001, 100001, 10007)

EndNodeSet = filter(lambda x: x.x == 0 and x.z == 0, Node.instances)

for i, obj in enumerate(EndNodeSet):
    LineElement.ForceBeamColumn(200001 + i, obj.n, 5, 1, 5, Int1, Sec1)

Point = filter(lambda x: x.x == 60 and x.z == 0 and x.y == 50, Node.instances)[0]
print('Point', Point.n)
SPConstraint(Point.n, 1, 1, 1, 1, 1, 1)

SPConstraint(5, 1, 1, 0, 1, 1, 1)

LoadSideNodes = filter(lambda x: x.x == 0 and x.z == 0, Node.instances)
map(lambda x: SPConstraint(x.n, 1, 1, 0, 1, 1, 1), LoadSideNodes)

PlaneNodes = filter(lambda x: x.z == 0, Node.instances)
PlaneNodes.remove(Point)
print('PlaneNodes', len(PlaneNodes))
Node100000 = filter(lambda x: x.n >= 100000 and x.n < 200000, PlaneNodes)
CopyNodes(Node100000, [0, 0, 0], 100000)
CopyNodes(CornerNodes, [0, 0, 0], 299000)
BaseNodes = filter(lambda x: x.n > 200000, Node.instances)
print('BaseNodes', len(BaseNodes))
for obj in BaseNodes:
    SPConstraint(obj.n, 1, 1, 1, 1, 1, 1)

XVec = Vector('XV', 3, [0, 0, 1])
YpVec = Vector('YpV', 3, [0, 1, 0])
MatList1 = LineElement.MaterialList([TensileOnlyMat])
DirList1 = LineElement.DirectionList([2])
for i, obj in enumerate(PlaneNodes):
    node2 = filter(lambda x: abs(x.x - obj.x) < tolerance and abs(x.y - obj.y) < tolerance and abs(x.z - obj.z) < tolerance, BaseNodes)[0]
    LineElement.ZeroLength(100000 + node2.n, node2.n, obj.n, MatList1, DirList1, XVec, YpVec)


LinearSeries = TimeSeries.Linear(1, 1.0)
...
theTest = AnalysisOption.Test.EnergyIncrement(1e-9, 400)
...
AnalysisOption.Analyze(theAnalysis, [100, 0.01, 0.0001, 0.01, 100])
